...
const softVariant: Record<string, ComponentsVariants<Theme>['MuiButtonGroup']> =
  {
    colors: COLORS.map((color) => ({
      ...
      style: ({ theme }) => ({
        [buttonClasses]: {
          borderColor: varAlpha(theme.palette[color].darkerChannel, 0.24),
          [stylesMode.dark]: {
            borderColor: varAlpha(theme.palette[color].lighterChannel, 0.24),
          },
        },
    ...
    base: [
      {
        ...
        style: ({ theme }) => ({
          [buttonClasses]: {
            borderRight: `solid 1px ${varAlpha(theme.palette.grey['500Channel'], 0.32)}`,
          ...
          [`&.${buttonGroupClasses.vertical}`]: {
            [buttonClasses]: {
              borderRight: 'none',
              borderBottom: `solid 1px ${varAlpha(theme.palette.grey['500Channel'], 0.32)}`,
              [`&.${buttonGroupClasses.disabled}`]: {
                borderColor: theme.palette.action.disabledBackground,
              },
            },
    ...
    contained: ({ theme, ownerState }) => {
      const styled = {
        colors: styleColors(ownerState, (color) => ({
          [buttonClasses]: {
            borderColor: varAlpha(theme.palette[color].darkerChannel, 0.48),
          },
        })),
        inheritColor: {
